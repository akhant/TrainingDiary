...
export default (state = [], action) => {
  const { /* pickDate, */ res, type } = action;
 
  if (!res) return state;

  if (action.type === FETCH_DATA) {
    state = res.approaches;
    return state;
  }

  if (type === ADD_APPROACH) {
      if (res.newApproach) {
        return state.concat([res.newApproach]);
      } else {
  ...
  if (type === CHANGE_NAME) {
    if (res.approaches[0])
    return state.filter(approach => approach.exerciseId !== res.approaches[0].exerciseId).concat(res.approaches)
  }
